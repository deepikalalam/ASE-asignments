...
var app = express();
var output={'body': []};
var url = 'mongodb://man1:human@ds161495.mlab.com:61495/tuto8';
app.use(cors());
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, data, callback) {
        db.collection('ase10').insertOne( data, function(err, result) {
            if(err)           {
...
app.post('/get-data',function (req,res) {
    ...
    var findUser = function(db, callback) {
        var cursor =db.collection('ase10').find();
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('ase10').updateOne({"_id":ObjectIdentity(id)},{$set:item}, function(err, result) {
            if(err)            {
                res.write("Registration Failed");
                res.end();            }
...
app.post('/delete', function(req, res) {
    ...
    MongoUser.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('ase10').deleteOne({"_id": ObjectIdentity(id)}, function(err, result) {
